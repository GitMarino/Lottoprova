...
public interface PersonDAO extends JpaRepository<Person, Long> {
	...
	@Query("SELECT p "
			+ "FROM person p left join PersonAreaConnection pa on p.id=pa.id.person.id left join area a on pa.id.area.id=a.id "
	...
	List<Person> search(@Param("model") SearchPeopleObject searchPeopleObject);
